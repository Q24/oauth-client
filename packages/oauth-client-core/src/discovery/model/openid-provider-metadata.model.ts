...
export interface OpenIDProviderMetadata {
   ...
   * available, a use (Key Use) parameter value is or all keys in the referenced
   * JWK Set to indicate each key's intended usage. Although some algorithms
   * allow the same key to be used for both signatures and encryption, doing so
   * is NOT RECOMMENDED, as it is less secure. The JWK x5c parameter MAY be used
   ...
   * this OP supports, as specified in OAuth 2.0 Multiple Response Type Encoding
   ...
   * OP supports. Dynamic OpenID Providers MUST support the authorization_code
   * and implicit Grant Type values and MAY support other Grant Types. If
   ...
   * The algorithm RS256 MUST be included. The value none MAY be supported, but
  ...
  /**
   * JSON array containing a list of the JWS [JWS] signing algorithms (alg
   * values) [JWA] supported by the UserInfo Endpoint to encode the Claims in a
   * JWT [JWT]. The value none MAY be included.
   */
   ...
   * of OpenID Connect Core 1.0 [OpenID.Core]. These algorithms are used both
  ...
  /**
   * JSON array containing a list of the JWE encryption algorithms (alg values)
   * supported by the OP for Request Objects. These algorithms are used both
   * when the Request Object is passed by value and when it is passed by
   * reference.
   */
   ...
   * in Section 9 of OpenID Connect Core 1.0 [OpenID.Core]. Other authentication
   ...
   * JSON array of BCP47 [RFC5646] language tag values.
   */
   ...
   * data provided by the OP. The registration process SHOULD display this URL
  ...
  /**
   * URL that the OpenID Provider provides to the person registering the Client
   * to read about OpenID Provider's terms of service. The registration process
   * SHOULD display this URL to the person registering the Client if it is
   * given.
   */
  op_tos_uri?: string;
}
